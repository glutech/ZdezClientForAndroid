...
public class ZdezMsgItem extends RelativeLayout {
	...
	public ZdezMsgItem(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.context = context;
	}

	public void setZdezMsgItem(ZdezMsgVo zdez) {
		findView();

		title.setText(zdez.getTitle());
		// 根据消息的未读状态设置不同的字体颜色
		if (zdez.getReadStatus() == 0) {
		...
		String[] dateStrs = ttw
				.getWordsOfTime(Timestamp.valueOf(zdez.getDate()));
		createdTime.setText(dateStrs[0]);

		// 设置图片，在没有封面时，去掉图片view
		Log.d(TAG, "getted msg coverPath:" + zdez.getCoverPath());
		if (null != zdez.getCoverPath() && !"".equals(zdez.getCoverPath())) {
			// 有封面图片
			Log.d(TAG, "太好了，有图片可以用来做封面;coverPath:" + zdez.getCoverPath());
			cover.setImageUrl(zdez.getCoverPath());
		} else {
			// 没有封面图片
